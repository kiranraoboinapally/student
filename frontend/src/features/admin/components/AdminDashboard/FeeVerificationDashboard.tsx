...
export default function FeeVerificationDashboard({
    ...
    const [statusFilter, setStatusFilter] = useState<"all" | "pending" | "verified" | "rejected" | "offline">("all");
    const [searchTerm, setSearchTerm] = useState("");
    const [processingId, setProcessingId] = useState<number | null>(null);

    // --- Normalize status function ---
    const normalizeStatus = (status: string | null): "pending" | "verified" | "offline" | "rejected" => {
        if (!status) return "offline";
        const s = status.toLowerCase();
        if (s === "paid" || s === "pending") return "pending";
        if (s === "status_success") return "verified";
        if (s === "rejected") return "rejected";
        return "pending";
    };

    // --- Filtered payments based on normalized status ---
    const filteredData = useMemo(() => {
        return localPayments.filter(payment => {
            const normStatus = normalizeStatus(payment.status);
    ...
    }, [localPayments, students, institutes, selectedInstitute, selectedCourse, statusFilter, searchTerm]);

    // Keep localPayments in sync with props
    useEffect(() => {
    ...
    // Stats
    const stats = useMemo(() => {
        const total = filteredData.reduce((acc, curr) => acc + (Number(curr.fee_amount) || 0), 0);
        const pendingCount = filteredData.filter(p => normalizeStatus(p.status) === "pending").length;
        const verifiedCount = filteredData.filter(p => normalizeStatus(p.status) === "verified").length;
    ...
    const uniqueInstitutes = useMemo(() => {
        ...
        institutes.forEach(inst => {
            ...
            if (id != null && !map.has(id)) map.set(id, inst);
        });
                ...
                <div className="relative">
                    ...
                    >
                        ...
                        {courses
                            ...
                            .map(course => <option key={course.course_id} value={course.course_id}>{course.name}</option>)}
                    </select>
                    ...
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {filteredData.length > 0 ? filteredData.map(payment => {
                                const student = students.find(s =>
                                    s.enrollment_number == (payment.student_id as any) || s.user_id == (payment.student_id as any)
                                );
                                const normalized = normalizeStatus(payment.status);
                                const showActions = normalized === "pending"; // Only show actions for pending
                                ...
                                return (
                                    <tr key={payment.payment_id} className="hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 font-bold text-gray-900">₹{(payment.fee_amount ?? 0).toLocaleString("en-IN")}</td>
                                        <td className="px-6 py-4 text-gray-600">{payment.paid_at ? new Date(payment.paid_at).toLocaleDateString() : '-'}</td>
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold
                                                ${normalized === 'verified' ? 'bg-emerald-100 text-emerald-800' :
                                                    normalized === 'rejected' ? 'bg-red-100 text-red-800' :
                                                        normalized === 'pending' ? 'bg-amber-100 text-amber-800' :
                                                            'bg-gray-100 text-gray-700'}`}>
                                                {normalized === 'verified' && <CheckCircle size={14} />}
                                                {normalized === 'rejected' && <XCircle size={14} />}
                                                {normalized === 'pending' && <AlertCircle size={14} />}
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            {showActions && (
                                                <div className="flex justify-end gap-2">
                                                    <button
                                                        onClick={() => handleVerify(payment.payment_id!, payment.source)}
                                                    ...
                                                    <button
                                                        onClick={() => handleReject(payment.payment_id!, payment.source)}
                                                        disabled={processingId === payment.payment_id}
                                ...
                                <tr>
                                    <td colSpan={7} className="px-6 py-4 text-center text-gray-500 italic">No payments found.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
